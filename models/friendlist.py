...
class FriendList(db.Model):
    ...
    user2_id = Column(Integer, ForeignKey("users.id"), nullable=False)  # Request receiver
    status = Column(String, default="pending")  # 'pending', 'accepted', or 'rejected'

    # Define relationships to User
    user1 = relationship("User", foreign_keys=[user1_id], backref="friend_requests_sent")
    user2 = relationship("User", foreign_keys=[user2_id], backref="friend_requests_received")

    # Enforce unique friend pairs
    __table_args__ = (UniqueConstraint("user1_id", "user2_id", name="unique_friend_pair"),)
    ...
    def accept_friend_request(cls, sender_id, receiver_id):
        ...
        request = cls.query.filter_by(user1_id=sender_id, user2_id=receiver_id, status="pending").first()
        if request:
    ...
    def get_friends(cls, user_id):
        ...
        friends_as_user1 = cls.query.filter_by(user1_id=user_id, status="accepted").all()
        friends_as_user2 = cls.query.filter_by(user2_id=user_id, status="accepted").all()

        # Combine and return friend IDs
        friend_ids = [f.user2_id for f in friends_as_user1] + [f.user1_id for f in friends_as_user2]
        return friend_ids
